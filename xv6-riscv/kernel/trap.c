...
usertrap(void)
{
  ...
  // In the MLFQ build a timer tick no longer means "always give up the CPU". mlfq_tick() charges the
  // tick to this process and only makes it give up the CPU when its time slice is used up or a
  // higher-priority process is waiting, so a process can really run its whole slice. The default
  // round-robin build keeps the original behaviour of yielding on every tick.
#ifdef USE_MLFQ
  if (which_dev == 2)
    mlfq_tick();
#else
...
kerneltrap()
{
  ...
  // give up the CPU if this is a timer interrupt.
  // Same as in usertrap(): a tick that arrives while a process is running kernel code (for example
  // during a system call) still counts toward that process's time slice under MLFQ.
#ifdef USE_MLFQ
